...
EDGES_QUERY="""MATCH (a:Account)-[:SENT]->(t:Transaction)-[:RECEIVED_BY]->(b:Account) RETURN a.account_id AS source,b.account_id AS target,t.txn_id AS txn_id,t.amount AS amount"""
...
def build_directed_multigraph(conn):
    ...
    for r in fetch_transaction_edges(conn): g.add_edge(r["source"],r["target"],txn_id=r["txn_id"],amount=float(r["amount"] or 0))
    return g
